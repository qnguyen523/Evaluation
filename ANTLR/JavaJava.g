...
methodDeclaratorRest
    :   formalParameters ('[' {s("[");} ']'{s("]");})*
        ...
        (   methodBody {System.out.println("MCC: "+ (mmc+1));}
        |   ';'
        )
    ;
...
literal 
    ...
    |   StringLiteral {JavaMetrics.uniqueConstants.add("StringLiteral: "+$StringLiteral.text);}
    |   booleanLiteral
...
WS  :  (' '{ws++;}|'\r'{ws++;}|'\t'{ws++;}|'\u000C'{ws++;}|'\n'{ws++; line_of_code++;}) {$channel=HIDDEN;}
    ;

COMMENT
    :   '/*' ( options {greedy=false;} : .{commentcount++;} )* '*/' {$channel=HIDDEN; line_of_comments++; line_of_code++;}
    ;

LINE_COMMENT
    : '//' ~( ('\n' |'\r'))* {line_of_comments++;} '\r'? '\n' {$channel=HIDDEN; line_of_code++;}
    ;
